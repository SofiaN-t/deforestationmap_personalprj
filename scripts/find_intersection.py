# Since the target data are under NDA,we will check intersection between two alert sets
...
# Combine alerts -- when there is a target file available
...
def find_intersection(gpd1, gpd2):
    ...
    if check_overlap.shape[0] > 0:
        ...
        #  check_overlap['intersecting_with'] = 'Unknown' 
        ...
        intersection_result = check_overlap.drop(['level_1', 'deforestac']) # For the particular dfs
    else:
        # helper_df = pd.DataFrame(data={'Amazonian': ['No intersection'] * gpd1.shape[0],
        #                                          'RADD': ['No intersection'] * gpd1.shape[0]})
        helper_df = pd.DataFrame(data={'Intersection': ['No intersection']*gpd1.shape[0]})
        intersection_result = pd.concat([gpd1.drop(['value','geometry'], axis=1), helper_df], axis=1)

    return intersection_result

# Find intersection
intersection_df = find_intersection(radd_gdf, amaz_gdf)
